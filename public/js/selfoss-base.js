...
var selfoss = {
    ...
    logout: function() {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                selfoss.ui.showError($('#lang').data('logout_error') + ': ' +
    ...
    reloadTags: function() {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                selfoss.ui.showError($('#lang').data('tags_error') + ': ' +
                                     textStatus + ' ' + errorThrown);
            },
            complete: function() {
    ...
    refreshTags: function(tags) {
        ...
        if (selfoss.filter.tag) {
            if (!selfoss.db.isValidTag(selfoss.filter.tag)) {
                selfoss.ui.showError($('#lang').data('tag_unknown') + ': ' + selfoss.filter.tag);
            }
    ...
    refreshSources: function(sources) {
        ...
        if (selfoss.filter.source) {
            if (!selfoss.db.isValidSource(selfoss.filter.source)) {
                selfoss.ui.showError($('#lang').data('tag_unknown_source') + ': '
    ...
    markVisibleRead: function() {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                selfoss.ui.showError($('#lang').data('mark_items_error') + ': ' +
                                     textStatus + ' ' + errorThrown);
            }
        });
    }
